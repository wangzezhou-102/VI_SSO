...
public class BkContinueTrackingServiceImpl implements BkContinueTrackingService {
    ...
    public ResultVo getBkContinueTracking(BkContinueTrackingRequest bkContinueTrackingRequest) {
        ...
        response = JSON.parseObject(JSON.toJSONString(data), (Class<BaseResponse<JSONArray>>) response.getClass());

//        BaseResponse<JSONArray> search = tuSouSearch.search(baseRequest);

        ArrayList<SearchResponseData> searchResponseData = JSON.parseObject(JSON.toJSONString(response.getData()), new TypeReference<ArrayList<SearchResponseData>>() {
        });
