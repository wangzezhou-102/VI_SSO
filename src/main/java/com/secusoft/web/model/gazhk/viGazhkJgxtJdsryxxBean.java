package com.secusoft.web.model.gazhk;

public class viGazhkJgxtJdsryxxBean {
    private String zybh;

    private String objectId;

    private String xb;

    private String jg;

    private String whcd;

    private String zy;

    private String tssf;

    private String dpzl1;

    private String hjszdSf;

    private String hyzk;

    private String gj;

    private String hjszd;

    private String jgSf;

    private String xdfs1;

    private String jdff;

    private String xzzqhSf;

    private String xzzqhSs;

    private String jbxxbh;

    private String xm;

    private String xbFormat;

    private String csrq;

    private String zjlxFormat;

    private String zjhm;

    private String hyzkFormat;

    private String mzFormat;

    private String gjFormat;

    private String jgFormat;

    private String whcdFormat;

    private String zyFormat;

    private String sfFormat;

    private String tssfFormat;

    private String hjszdFormat;

    private String hjdxz;

    private String xzzqhFormat;

    private String xzzxz;

    private String xdfs1Format;

    private String dpzl1Format;

    private String jdffFormat;

    private String badw;

    private String bar;

    private String jyaq;

    private String rybh;

    private String rsyyFormat;

    private String rsrq;

    private String csyyFormat;

    private String outrq;

    private String csqx;

    private String rswsh;

    private String jsmc;

    private String rsyy;

    private String zjlx;

    private String mz;

    private String sf;

    private String xzzqh;

    private String hjszdSs;

    private String jgSs;

    private String jsbh;

    private String jsh;

    public String getZybh() {
        return zybh;
    }

    public void setZybh(String zybh) {
        this.zybh = zybh == null ? null : zybh.trim();
    }

    public String getObjectId() {
        return objectId;
    }

    public void setObjectId(String objectId) {
        this.objectId = objectId == null ? null : objectId.trim();
    }

    public String getXb() {
        return xb;
    }

    public void setXb(String xb) {
        this.xb = xb == null ? null : xb.trim();
    }

    public String getJg() {
        return jg;
    }

    public void setJg(String jg) {
        this.jg = jg == null ? null : jg.trim();
    }

    public String getWhcd() {
        return whcd;
    }

    public void setWhcd(String whcd) {
        this.whcd = whcd == null ? null : whcd.trim();
    }

    public String getZy() {
        return zy;
    }

    public void setZy(String zy) {
        this.zy = zy == null ? null : zy.trim();
    }

    public String getTssf() {
        return tssf;
    }

    public void setTssf(String tssf) {
        this.tssf = tssf == null ? null : tssf.trim();
    }

    public String getDpzl1() {
        return dpzl1;
    }

    public void setDpzl1(String dpzl1) {
        this.dpzl1 = dpzl1 == null ? null : dpzl1.trim();
    }

    public String getHjszdSf() {
        return hjszdSf;
    }

    public void setHjszdSf(String hjszdSf) {
        this.hjszdSf = hjszdSf == null ? null : hjszdSf.trim();
    }

    public String getHyzk() {
        return hyzk;
    }

    public void setHyzk(String hyzk) {
        this.hyzk = hyzk == null ? null : hyzk.trim();
    }

    public String getGj() {
        return gj;
    }

    public void setGj(String gj) {
        this.gj = gj == null ? null : gj.trim();
    }

    public String getHjszd() {
        return hjszd;
    }

    public void setHjszd(String hjszd) {
        this.hjszd = hjszd == null ? null : hjszd.trim();
    }

    public String getJgSf() {
        return jgSf;
    }

    public void setJgSf(String jgSf) {
        this.jgSf = jgSf == null ? null : jgSf.trim();
    }

    public String getXdfs1() {
        return xdfs1;
    }

    public void setXdfs1(String xdfs1) {
        this.xdfs1 = xdfs1 == null ? null : xdfs1.trim();
    }

    public String getJdff() {
        return jdff;
    }

    public void setJdff(String jdff) {
        this.jdff = jdff == null ? null : jdff.trim();
    }

    public String getXzzqhSf() {
        return xzzqhSf;
    }

    public void setXzzqhSf(String xzzqhSf) {
        this.xzzqhSf = xzzqhSf == null ? null : xzzqhSf.trim();
    }

    public String getXzzqhSs() {
        return xzzqhSs;
    }

    public void setXzzqhSs(String xzzqhSs) {
        this.xzzqhSs = xzzqhSs == null ? null : xzzqhSs.trim();
    }

    public String getJbxxbh() {
        return jbxxbh;
    }

    public void setJbxxbh(String jbxxbh) {
        this.jbxxbh = jbxxbh == null ? null : jbxxbh.trim();
    }

    public String getXm() {
        return xm;
    }

    public void setXm(String xm) {
        this.xm = xm == null ? null : xm.trim();
    }

    public String getXbFormat() {
        return xbFormat;
    }

    public void setXbFormat(String xbFormat) {
        this.xbFormat = xbFormat == null ? null : xbFormat.trim();
    }

    public String getCsrq() {
        return csrq;
    }

    public void setCsrq(String csrq) {
        this.csrq = csrq == null ? null : csrq.trim();
    }

    public String getZjlxFormat() {
        return zjlxFormat;
    }

    public void setZjlxFormat(String zjlxFormat) {
        this.zjlxFormat = zjlxFormat == null ? null : zjlxFormat.trim();
    }

    public String getZjhm() {
        return zjhm;
    }

    public void setZjhm(String zjhm) {
        this.zjhm = zjhm == null ? null : zjhm.trim();
    }

    public String getHyzkFormat() {
        return hyzkFormat;
    }

    public void setHyzkFormat(String hyzkFormat) {
        this.hyzkFormat = hyzkFormat == null ? null : hyzkFormat.trim();
    }

    public String getMzFormat() {
        return mzFormat;
    }

    public void setMzFormat(String mzFormat) {
        this.mzFormat = mzFormat == null ? null : mzFormat.trim();
    }

    public String getGjFormat() {
        return gjFormat;
    }

    public void setGjFormat(String gjFormat) {
        this.gjFormat = gjFormat == null ? null : gjFormat.trim();
    }

    public String getJgFormat() {
        return jgFormat;
    }

    public void setJgFormat(String jgFormat) {
        this.jgFormat = jgFormat == null ? null : jgFormat.trim();
    }

    public String getWhcdFormat() {
        return whcdFormat;
    }

    public void setWhcdFormat(String whcdFormat) {
        this.whcdFormat = whcdFormat == null ? null : whcdFormat.trim();
    }

    public String getZyFormat() {
        return zyFormat;
    }

    public void setZyFormat(String zyFormat) {
        this.zyFormat = zyFormat == null ? null : zyFormat.trim();
    }

    public String getSfFormat() {
        return sfFormat;
    }

    public void setSfFormat(String sfFormat) {
        this.sfFormat = sfFormat == null ? null : sfFormat.trim();
    }

    public String getTssfFormat() {
        return tssfFormat;
    }

    public void setTssfFormat(String tssfFormat) {
        this.tssfFormat = tssfFormat == null ? null : tssfFormat.trim();
    }

    public String getHjszdFormat() {
        return hjszdFormat;
    }

    public void setHjszdFormat(String hjszdFormat) {
        this.hjszdFormat = hjszdFormat == null ? null : hjszdFormat.trim();
    }

    public String getHjdxz() {
        return hjdxz;
    }

    public void setHjdxz(String hjdxz) {
        this.hjdxz = hjdxz == null ? null : hjdxz.trim();
    }

    public String getXzzqhFormat() {
        return xzzqhFormat;
    }

    public void setXzzqhFormat(String xzzqhFormat) {
        this.xzzqhFormat = xzzqhFormat == null ? null : xzzqhFormat.trim();
    }

    public String getXzzxz() {
        return xzzxz;
    }

    public void setXzzxz(String xzzxz) {
        this.xzzxz = xzzxz == null ? null : xzzxz.trim();
    }

    public String getXdfs1Format() {
        return xdfs1Format;
    }

    public void setXdfs1Format(String xdfs1Format) {
        this.xdfs1Format = xdfs1Format == null ? null : xdfs1Format.trim();
    }

    public String getDpzl1Format() {
        return dpzl1Format;
    }

    public void setDpzl1Format(String dpzl1Format) {
        this.dpzl1Format = dpzl1Format == null ? null : dpzl1Format.trim();
    }

    public String getJdffFormat() {
        return jdffFormat;
    }

    public void setJdffFormat(String jdffFormat) {
        this.jdffFormat = jdffFormat == null ? null : jdffFormat.trim();
    }

    public String getBadw() {
        return badw;
    }

    public void setBadw(String badw) {
        this.badw = badw == null ? null : badw.trim();
    }

    public String getBar() {
        return bar;
    }

    public void setBar(String bar) {
        this.bar = bar == null ? null : bar.trim();
    }

    public String getJyaq() {
        return jyaq;
    }

    public void setJyaq(String jyaq) {
        this.jyaq = jyaq == null ? null : jyaq.trim();
    }

    public String getRybh() {
        return rybh;
    }

    public void setRybh(String rybh) {
        this.rybh = rybh == null ? null : rybh.trim();
    }

    public String getRsyyFormat() {
        return rsyyFormat;
    }

    public void setRsyyFormat(String rsyyFormat) {
        this.rsyyFormat = rsyyFormat == null ? null : rsyyFormat.trim();
    }

    public String getRsrq() {
        return rsrq;
    }

    public void setRsrq(String rsrq) {
        this.rsrq = rsrq == null ? null : rsrq.trim();
    }

    public String getCsyyFormat() {
        return csyyFormat;
    }

    public void setCsyyFormat(String csyyFormat) {
        this.csyyFormat = csyyFormat == null ? null : csyyFormat.trim();
    }

    public String getOutrq() {
        return outrq;
    }

    public void setOutrq(String outrq) {
        this.outrq = outrq == null ? null : outrq.trim();
    }

    public String getCsqx() {
        return csqx;
    }

    public void setCsqx(String csqx) {
        this.csqx = csqx == null ? null : csqx.trim();
    }

    public String getRswsh() {
        return rswsh;
    }

    public void setRswsh(String rswsh) {
        this.rswsh = rswsh == null ? null : rswsh.trim();
    }

    public String getJsmc() {
        return jsmc;
    }

    public void setJsmc(String jsmc) {
        this.jsmc = jsmc == null ? null : jsmc.trim();
    }

    public String getRsyy() {
        return rsyy;
    }

    public void setRsyy(String rsyy) {
        this.rsyy = rsyy == null ? null : rsyy.trim();
    }

    public String getZjlx() {
        return zjlx;
    }

    public void setZjlx(String zjlx) {
        this.zjlx = zjlx == null ? null : zjlx.trim();
    }

    public String getMz() {
        return mz;
    }

    public void setMz(String mz) {
        this.mz = mz == null ? null : mz.trim();
    }

    public String getSf() {
        return sf;
    }

    public void setSf(String sf) {
        this.sf = sf == null ? null : sf.trim();
    }

    public String getXzzqh() {
        return xzzqh;
    }

    public void setXzzqh(String xzzqh) {
        this.xzzqh = xzzqh == null ? null : xzzqh.trim();
    }

    public String getHjszdSs() {
        return hjszdSs;
    }

    public void setHjszdSs(String hjszdSs) {
        this.hjszdSs = hjszdSs == null ? null : hjszdSs.trim();
    }

    public String getJgSs() {
        return jgSs;
    }

    public void setJgSs(String jgSs) {
        this.jgSs = jgSs == null ? null : jgSs.trim();
    }

    public String getJsbh() {
        return jsbh;
    }

    public void setJsbh(String jsbh) {
        this.jsbh = jsbh == null ? null : jsbh.trim();
    }

    public String getJsh() {
        return jsh;
    }

    public void setJsh(String jsh) {
        this.jsh = jsh == null ? null : jsh.trim();
    }
}