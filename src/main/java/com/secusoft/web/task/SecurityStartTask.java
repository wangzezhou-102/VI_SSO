...
import static com.secusoft.web.utils.SecurityTaskUntils.securityTaskBeginTime;

/**
 * 启动布控定时任务
 *
 */
public class SecurityStartTask extends TimerTask {
    private static Logger log = LoggerFactory.getLogger(SecurityStartTask.class);

    private SecurityTaskRequest securityTaskRequest;

    public SecurityStartTask(SecurityTaskRequest securityTaskRequest) {
    ...
    @Override
    public void run() {
        log.info("开始启动安保任务，安保任务编号：" + securityTaskRequest.getTaskId());
        if (securityTaskRequest != null && securityTaskRequest.getId() != null) {
            log.info("开始对比时间是否一致");
            if (!securityTaskBeginTime(securityTimeMapper, securityTaskRequest)) {
                log.info("时间不一致，无法启流，安保任务编号：" + securityTaskRequest.getTaskId());
                return;
            }

            if (securityTaskRequest != null && 2 == securityTaskRequest.getEnable()) {
                ...
                securityTaskRequest.setEnable(1);
                //判断返回值code，若开启任务成功，则更改布控任务状态为1
                if (String.valueOf(BizExceptionEnum.OK.getCode()).equals(code)) {
                    log.info("任务号：" + securityTaskRequest.getTaskId() + "，开启任务成功");
                    securityTaskRequest.setSecurityStatus(1);
                } else {
                    log.info("任务号：" + securityTaskRequest.getTaskId() + "，开启任务失败，原因：" + message);
                ...
                if (0 == securityTaskRequest.getSecurityStatus()) {
                    throw new RuntimeException("任务号：" + securityTaskRequest.getTaskId() + "，开启任务失败，原因：" + message);
                }
            }
        }
    }
