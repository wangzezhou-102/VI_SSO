...
UserInfoDlg.onClickRole = function (e, treeId, treeNode) {
    $("#roleSel").attr("value", role_instance.getSelectedVal());
...
function onBodyDown(event) {
    if (!(event.target.id == "menuBtn" || event.target.id == "menuContent" || $(
            event.target).parents("#menuContent").length > 0)||event.target.id == "rolemenuContent" || $(
...
$(function () {
    ...
    ztree.init();
    instance = ztree;
    var roleZtree = new $ZTree("roleTree", "/role/roleTreeList");
    roleZtree.bindOnClick(UserInfoDlg.onClickRole);
    roleZtree.init();
    role_instance = roleZtree;
